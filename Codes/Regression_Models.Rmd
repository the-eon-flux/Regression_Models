...
knots <- c(mean(mtcars$disp), mean(mtcars$disp)+sd(mtcars$disp))

splineTerms <- sapply(knots, function(knot){ (mtcars$disp > knot)* (mtcars$disp - knot) })

splineTerms1 <- sapply(knots, function(knot){ifelse(mtcars$disp - knot > 0, mtcars$disp - knot, 0) })

xMat <- cbind(Intercept = 1, Displacement = mtcars$disp, splineTerms )
colnames(xMat) <- c( "Intercept", "Displacement", "Knot1", "Knot2")

fit2 <- lm(mtcars$qsec ~ xMat - 1)
yhat <- predict(fit2)

plot(mtcars$disp, mtcars$qsec, pch = 16, xlab = "Displacement", ylab = "qsec (1/4 mile time)" )
lines(mtcars$disp[1:2], yhat[1:2], lwd = 2, col = "dodgerblue")
